...
export class Organization {
  ...
  // ИНН — используется Telegram-ботом самостоятельной регистрации (telegram-bot/) для
  // сопоставления "название + ИНН" с конкретной организацией. Nullable — заполняется по
  // мере необходимости, не все организации обязаны регистрировать через бота. unique как
  // подстраховка от случайного дубля (Postgres допускает сколько угодно NULL при
  // unique-констрейнте, конфликтуют только заполненные значения).
