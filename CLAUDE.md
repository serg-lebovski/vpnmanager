...
- `installProtocol` перед SSH проверяет, что порт/сеть не заняты ДРУГИМ протоколом того же
  сервера (форма установки на фронтенде подставляет для любого протокола одинаковые
  порт/сеть по умолчанию, если ничего не подобрано умнее — легко столкнуть два протокола)
  — явная ошибка вместо малопонятной "RTNETLINK answers: Address already in use" на SSH-
  уровне. Плюс перед `${quickBinary} up` драйвер сам сносит осиротевший wireguard-
  совместимый интерфейс, если он уже держит нужный шлюзовой IP (`reclaimConflictingInterface`
  в `base-wireguard-like.driver.ts`, по link type "wireguard" в `ip -d link show` — ловит
  и WG, и AWG, это форк ядра WireGuard) — раньше такой конфликт требовал ручной правки по
  SSH (см. текст ошибки-подсказки в `install()`, которая остаётся подстраховкой, если
  автоочистка не сработала).
- **Kernel-модуль протокола собран не под то ядро** (`VpnProvisioningService.
  ensureKernelModuleReady`, пойманный вживую реальный случай) — `<quickBinary> up` падает с
  "Unknown device type", когда DKMS-модуль протокола (обычно AmneziaWG) физически собран и
  установлен, но для ДРУГОЙ версии ядра, чем сейчас реально загружена на сервере (типичная
  причина: `apt upgrade` подтянул новее kernel-пакет, DKMS пересобрал модуль под него, но
  сервер ни разу не перезагружался). `VpnDriver.checkKernelModuleStatus` (`modprobe` +
  при неудаче парсинг `dkms status`) определяет именно этот случай ДО открытия основной
  SSH-сессии `install()`/`connectAsClient()` — если да, `ensureKernelModuleReady` сама
  отправляет `reboot`, дожидается возврата SSH-порта (`common/tcp-probe.util.ts`, до 150с —
  с запасом под `proxy_read_timeout 300s` на `/api/`) и перепроверяет готовность модуля,
  прежде чем продолжать install. Если модуль вообще не собран (не только не под то ядро) —
  перезагрузка не поможет, ничего не делаем и даём упасть как раньше с ручной подсказкой
  (`withKernelModuleHint` в `base-wireguard-like.driver.ts`: `modprobe`/`dkms status`/
  переустановка с заголовками ядра). Это перезагрузка ВСЕГО сервера — другие уже активные
  протоколы/peers на нём на время (обычно 30-90с) тоже станут недоступны, это неизбежное
  следствие смены ядра, не обойти без реальной перезагрузки.
- Удаление ОДНОГО протокола (`DELETE /servers/protocols/:id`, `ServersService.removeProtocol`)
  — в отличие от удаления самого сервера (SSH намеренно не трогает — сервер обычно удаляют
  именно потому, что недоступен) — реально снимает интерфейс на сервере (`VpnDriver.
  uninstall`: down, отключение автозапуска, конфиг/ключи), best-effort (сервер может быть
  недоступен именно из-за того, что протокол и удаляют — не блокирует очистку БД).
  Заблокировано для клиентского интерфейса моста (структурная часть моста — удаляется
  только вместе с ним); если протокол служит upstream — сначала переключает зависящие
  мосты на другой сервер того же VPN-протокола (`BridgesService.reassignUpstreamAwayFrom`,
  тот же путь, что при удалении сервера).
- `ServerProtocol.packageVersion` — версия CLI (`wg --version`/`awg --version`),
  записывается при установке (в той же SSH-сессии, без лишнего подключения) и по кнопкам
  «Проверить версию»/«Обновить пакет» на фронтенде (`VpnDriver.getInstalledVersion`/
  `updatePackage` — `apt-get install --only-upgrade`, обновление в рамках уже подключённых
  источников/PPA, не смена мажорной версии). Недоступно для протоколов в стороннем Docker-
  контейнере (официальный self-hosted сервер AmneziaVPN) — `execContainer` задан.
...
**Доступ организации к серверам/мостам** (`Organization.allowedServerIds`/`blockedBridgeIds`,
настраивается суперадмином на странице клиента) — асимметричные списки, не зеркальные: обычные
серверы — allow-list (`allowedServerIds`, по умолчанию пусто — org_admin/org_user вообще не может
выбрать конкретный сервер напрямую, только через мост), мосты — block-list (`blockedBridgeIds`, по
умолчанию пусто — доступны ВСЕ видимые организации мосты, общие+свои). Асимметрия намеренная: мост
— штатный, "безопасный" путь клиента (сам смысл моста — не давать клиентам подключаться к backend-
серверам напрямую), поэтому доступен по умолчанию; прямой выбор сервера — более привилегированная
опция в обход моста, выдаётся явно. Проверяется в `PeersService.create` (пропускается для
`SUPER_ADMIN` — ограничения только для self-service ролей) и попутно скрывается из выдачи:
`BridgesService.findAll` не возвращает заблокированные мосты, `PeersService.
getAllowedServersForRequester` (`GET /peers/allowed-servers`, доступен всем ролям в отличие от
`/servers`) — только разрешённые серверы. Автобаланс без явного выбора моста/сервера
(`LoadBalancerService.pickServerProtocol`) для таких ролей ограничен тем же `allowedServerIds`, а
не глобальным пулом серверов протокола.
...
**Обход upstream по доменам/IP** (`Bridge.bypassDestinations`, суперадмин, настройки моста) —
список доменов/IP-адресов, трафик клиентов моста к которым идёт НАПРЯМУЮ с self-сервера, минуя
upstream ("зарубежный" сервер); редактируется как текст (по записи на строку) или загружается
из `.txt` (дозаполняет список на фронтенде, дедуп на бэкенде). Механизм (`VpnProvisioningService.
setupBridgeBypass`): IP/CIDR используются как есть, домены резолвятся на self-сервере (`getent
ahostsv4`, все A-записи, не только первая — важно для CDN) и вместе с IP складываются в `ipset`
(`ipset swap` — атомарная замена содержимого без "окна" с пустым набором); `iptables` (mangle
PREROUTING) метит пакеты с dst из этого ipset, а `ip rule fwmark ... lookup main priority 90`
(общее для всех мостов на self-сервере правило, priority ниже/выше приоритетом, чем `from
<cidr> lookup <routeTable> priority 100` из `setupBridgeNat`) отправляет их через main вместо
routeTable моста — независимо от того, какой upstream сейчас активен (manual/auto/failover).
Отдельные `MASQUERADE`/`FORWARD ACCEPT` для этого пути — приватный IP клиента иначе не смог бы
выйти в интернет напрямую; обратное направление пропускается по `ESTABLISHED,RELATED`, не по
метке (mark не переживает новый проход ответных пакетов через PREROUTING). Список применяется
сразу при сохранении (best-effort, self-сервер может быть временно недоступен — список в БД не
откатывается) и переприменяется раз в 5 минут (`BridgesService.refreshBypassRules`) — домены
могут поменять IP (CDN) без какого-либо действия администратора.

**Дашборд и история трафика** (`dashboard/`) — живой снапшот (`DashboardService.poll`, `@Interval`
раз в 7с) идёт клиенту только по WebSocket (namespace `dashboard`, только `super_admin`, см.
`dashboard.gateway.ts`) и нигде не сохраняется, кроме `lastSnapshot` в памяти процесса. История
трафика для отчётов "день/неделя/месяц/помесячно" на дашборде — отдельная таблица
`PeerTrafficSample`: раз в `PERSIST_INTERVAL_MS` (15 мин) из уже полученного (без лишнего SSH-
запроса) снапшота сохраняется ДЕЛЬТА (не кумулятивный счётчик) трафика каждого peer'а —
`max(0, …)` защищает от отрицательной дельты при сбросе счётчиков интерфейса (переустановка/
down-up), тем же способом, что и мгновенный bps в `buildPeerStats`. Строки не FK на Peer/Server
(денормализованы: `peerName`/`serverName`/`organizationId`/`peerSource` — снимок на момент
замера) — история должна пережить удаление peer'а/сервера, а не исчезнуть вместе с ним.
`GET /dashboard/traffic/{by-server,by-organization,by-peer,monthly}` (`SUPER_ADMIN`-only, та
же граница видимости, что у самого WS) агрегируют `SUM()` по диапазону; `by-server`/`monthly`
(обе группируют по серверу, `monthly` — ещё и по календарному месяцу) намеренно НЕ исключают
`PeerSource.BRIDGE_UPSTREAM` — это реальный трафик через тот сервер (self-сервер моста,
в частности), а вот `by-organization`/`by-peer` его исключают — системный peer моста не
настоящий клиент и так скрыт из обычных списков peers (см. `PeersService`); без исключения
весь транзитный трафик моста осел бы в «Без клиента» и искажал разбивку по клиентам.
`by-organization` — разбивка по клиентам (`Organization.name`, резолвится отдельным запросом
по встреченным `organizationId`, а не JOIN'ом — `PeerTrafficSample` не хранит имя организации
денормализованным, в отличие от peer/server; null → «Без клиента», организация с тех пор
удалённая → «Организация удалена»). Все четыре эндпоинта на фронтенде (`DashboardPage.tsx`,
`TrafficSection`) принимают необязательные `organizationId`/`serverId` — фильтры для удобства
мониторинга конкретного клиента и/или сервера, применяются перекрёстно (кроме `by-organization`,
которую фильтр по клиенту тривиально схлопнул бы в одну строку — там смысл сохраняет только
фильтр по серверу).
...
**Журнал действий** (`audit-log/`, вкладка «Журнал действий» в меню суперадмина) —
`AuditLogInterceptor` зарегистрирован ГЛОБАЛЬНО (`APP_INTERCEPTOR`), а не расставлен вручную
по сервисам: ловит любой POST/PATCH/PUT/DELETE на защищённый `JwtAuthGuard`'ом маршрут (плюс
явный allowlist чувствительных GET, сейчас — `/system/backup*`), логирует метод+путь+`params.id`
как есть, БЕЗ ручного маппинга в понятные названия действий на каждый из 20+ эндпоинтов —
осознанный компромисс ширины покрытия против читаемости (действие видно по методу+пути в
интерфейсе). Тело запроса перед сохранением рекурсивно чистится от `password`/`secret`/
`privateKey`/`confirmationPhrase` (`AuditLogService.redact`) — журнал не должен стать ещё одним
местом, где лежит расшифрованный секрет. `/auth/*` и `/health` не логируются вовсе (до
аутентификации `request.user` ещё нет; неудачные попытки входа отдельно уходят в обычный
`Logger`, см. rate-limit/блокировку в `AuthService`). Запись — best-effort (падение записи не
роняет сам запрос) и только по УСПЕШНОМУ ответу (`tap`, не сработает при выброшенном исключении).
...
**Backlog (запланировано, не реализовано)**: веб-терминал SSH к добавленным серверам прямо из
панели (пользователем явно понижен приоритет — "не приоритет, можешь пока просто запланировать");
если возьмёшься за это, переиспользуй расшифровку `Server.sshSecretEnc`/`connectionParams` из
`VpnProvisioningService`, для передачи потока команд подойдёт WebSocket-gateway по аналогии с
`dashboard.gateway.ts`/`system` namespace (прогресс-события), а не HTTP.
...
Ещё один пойманный вживую инцидент (2026-08-07): `runUpdateSequence` раньше явно вызывала
`NginxConfigService.render()` сразу после `git pull` + сборки образов, НО ДО пересоздания
самого backend — то есть уже СВЕЖИЙ `nginx.conf.template` (`git pull` его обновил) рендерился
ЕЩЁ СТАРЫМ кодом `render()` (текущий процесс — это всё ещё старый backend, новый образ только
собран, но не запущен, см. `recreateBackendDetached`). Если очередной коммит меняет
`nginx.conf.template` и `NginxConfigService.render()` СОВМЕСТНО (как в коммите, добавившем
`limit_req` в шаблон и `limit_req_zone` в render() одним пакетом) — старый `render()` не знает
про новую `limit_req_zone`-декларацию, на выходе получается `limit_req zone=X` без единой
`limit_req_zone` — nginx падает на `nginx -t` с `zero size shared memory zone` уже на этапе
выделения shared-памяти (не синтаксическая ошибка, поэтому "syntax is ok" всё равно печатается).
Вся последовательность обновления при этом обрывается ДО пересоздания nginx/frontend/backend —
пользователь остаётся на старой сборке (в частности, без только что задеплоенных фронтенд-фич).
Фикс: явный вызов `render()` убран из `runUpdateSequence` целиком — код и шаблон гарантированно
из одного коммита только внутри `NginxConfigService.onModuleInit()` УЖЕ ПЕРЕСОЗДАННОГО backend
(шаг `recreateBackendDetached`), туда рендер и перенесён по факту (он и раньше там был, просто
дублировался мидсеквенс-вызовом). До перезапуска backend nginx продолжает работать на своём
прежнем, валидном конфиге — потерь функциональности нет, только небольшая задержка до
переприменения.
