...
`GET /dashboard/traffic/{by-server,by-peer,monthly}` (`SUPER_ADMIN`-only, та же граница
видимости, что у самого WS) агрегируют `SUM()` по диапазону; `by-server`/`monthly` (обе
группируют по серверу, `monthly` — ещё и по календарному месяцу) намеренно НЕ исключают
