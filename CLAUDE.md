...
пути в `docker-compose.yml` (build-контексты, volume-мапинги) резолвятся неверно. Команда
самообновления включает `--force-recreate`: без него сервис с неизменившимся образом (у нас
так `nginx`) не пересоздаётся, даже если поменялся смонтированный ОТДЕЛЬНЫЙ ФАЙЛ конфига
(`nginx/nginx.conf`) — `git pull` заменяет файл новым inode, а bind-mount уже запущенного
контейнера продолжает указывать на старый (поймано вживую при первом деплое этой фичи
2026-08-03).
